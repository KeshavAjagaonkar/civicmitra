...
exports.getWorkerPerformance = asyncHandler(async (req, res, next) => {
  ...
  const workerPerformanceData = await Promise.all(
    workers.map(async (worker) => {
      ...
      // Compute real average rating from Feedback model
      const workerComplaints = await Complaint.find({ workerId: worker._id }).select('_id');
      const complaintIds = workerComplaints.map(c => c._id);
      const feedbackAgg = await Feedback.aggregate([
        { $match: { complaintId: { $in: complaintIds } } },
        { $group: { _id: null, avgRating: { $avg: '$rating' }, count: { $sum: 1 } } }
      ]);
      const rating = feedbackAgg.length > 0 ? Math.round(feedbackAgg[0].avgRating * 10) / 10 : 0;
